/* ============================================================
   Planner Pro — Экраны приложения  (v1.0.1)
   Dashboard, Tasks, Calendar, Notes, NoteEditor, Projects,
   Stats, Settings, Trash
   ------------------------------------------------------------
   Каждый вью может вернуть cleanup-функцию для снятия слушателей
   (используется роутером через currentCleanup)
============================================================ */
(function () {
  'use strict';
  ...
  function openTaskModal(taskId = null, defaults = {}) {
    ...
    const content = `
      ...
      ${fieldRow('Подзадачи', `<div id="tf-subtasks"></div>
        <div style="display:flex;gap:8px;margin-top:6px">
          <input class="input" id="tf-subtask-input" placeholder="Новая подзадача…" style="flex:1">
          <button class="btn btn-icon btn-ghost" id="tf-subtask-add" aria-label="Добавить подзадачу">${I.get('plus', 18)}</button>
        </div>`)}
      ${fieldRow('Теги', `<div id="tf-tags"></div>`)}
    ...
    C.Modal.open({
      ...
      onMount(sheet, close) {
        ...
        $('#tf-date').value = task?.dueDate || defaults.dueDate || '';
        ...
        /* ---------- Подзадачи ---------- */
        let subtasks = task?.subtasks ? task.subtasks.map((s) => ({ ...s })) : [];
        const subtasksWrap = $('#tf-subtasks');

        function renderSubtasks() {
          subtasksWrap.innerHTML = '';
          subtasks.forEach((sub) => {
            subtasksWrap.appendChild(C.subtaskItem(sub, {
              onToggle(s) { s.completed = !s.completed; renderSubtasks(); },
              onRemove(s) { subtasks = subtasks.filter((x) => x.id !== s.id); renderSubtasks(); }
            }));
          });
        }
        ...
        $('#tf-subtask-add').addEventListener('click', () => {
          ...
          if (!title) return;
          subtasks.push({ id: U.uuid(), title, completed: false });
          input.value = '';
          renderSubtasks();
        });
        $('#tf-subtask-input').addEventListener('keydown', (e) => {
          if (e.key === 'Enter') { e.preventDefault(); $('#tf-subtask-add').click(); }
        });
        renderSubtasks();

        /* ---------- Теги ---------- */
        let currentTags = task?.tags ? [...task.tags] : [];
        const tagNames = Store.state.tags.map((t) => t.name);
        const tagsWrap = $('#tf-tags');
        tagsWrap.appendChild(C.tagInput({
          tags: currentTags,
          suggestions: tagNames,
          onChange(tags) { currentTags = tags; }
        }));
        ...
        sheet.querySelectorAll('input').forEach((inp) =>
          inp.addEventListener('keydown', (e) => {
            if (e.key === 'Enter' && inp.type !== 'date' && inp.type !== 'time') save();
          })
        );
  ...
  Views.dashboard = function (container) {
    ...
    const completedToday = stats.byDay[U.todayISO()] || 0;
    ...
    container.innerHTML = `
      ...
      <section class="dashboard-hero">
        ...
        <div>
          ...
          <div style="font-size:32px;font-weight:800;line-height:1.2">${completedToday} / ${totalToday}</div>
          <div style="font-size:13px;opacity:.85;margin-top:4px">
            ${stats.overdueCount ? `${stats.overdueCount} просрочено` : 'Всё под контролем'}
          </div>
        </div>
      </section>
    ...
    container.querySelector('[data-ring]').appendChild(C.progressRing(progressPct));
    ...
    // Pull-to-refresh с индикатором и cleanup
    const host = container.parentElement;
    const indicator = document.createElement('div');
    indicator.className = 'ptr-indicator';
    indicator.innerHTML = `${I.get('refresh', 18)} Обновление…`;
    host.prepend(indicator);

    U.attachPullToRefresh(host, async () => {
      await Promise.all([S.TaskService.loadAll(), S.NoteService.loadAll()]);
      Toast.success('Обновлено');
    });

    return () => indicator.remove(); // cleanup для роутера
  };
  ...
  Views.tasks = function (container) {
    ...
    function renderChips() {
      ...
      Object.entries(prio).forEach(([key, p]) => {
        ...
        const chip = el(`<button class="chip ${active ? 'active' : ''}"><span class="project-dot" style="background:${p.color}"></span>${p.label}</button>`);
      ...
      Store.state.projects.forEach((pr) => {
        ...
        const chip = el(`<button class="chip ${active ? 'active' : ''}"><span class="project-dot" style="background:${esc(pr.color)}"></span>${esc(pr.name)}</button>`);
        chip.addEventListener('click', () => {
  ...
  function openNoteEditor(noteId = null) {
    ...
    const content = `
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px">
        <input class="input" id="ne-title" placeholder="Заголовок" style="flex:1;font-weight:700;font-size:18px;border:none;background:transparent;padding-left:0">
        <button class="btn btn-icon btn-ghost" id="ne-favorite" aria-label="Избранное" style="color:${note?.isFavorite ? 'var(--accent)' : 'var(--text-tertiary)'}">
          ${I.get(note?.isFavorite ? 'star' : 'star', 20)}
        </button>
      </div>
    ...
    C.Modal.open({
      ...
      onMount(sheet, close) {
        ...
        const favBtn = sheet.querySelector('#ne-favorite');
        let isFav = note?.isFavorite || false;

        titleInput.value = note?.title || '';
        // Санитизация при загрузке контента (защита от XSS через импорт)
        editor.innerHTML = U.sanitizeHTML(note?.content || '');

        favBtn.addEventListener('click', async () => {
          isFav = !isFav;
          favBtn.style.color = isFav ? 'var(--accent)' : 'var(--text-tertiary)';
          if (note) await S.NoteService.toggleFavorite(note.id);
          U.haptic(10);
        });

        const tools = [
          ['bold', 'Жирный', () => document.execCommand('bold')],
          ['italic', 'Курсив', () => document.execCommand('italic')],
          ['strikethrough', 'Зачёркнутый', () => document.execCommand('strikeThrough')],
          ['heading', 'Заголовок 1', () => document.execCommand('formatBlock', false, 'H1')],
          ['heading', 'Заголовок 2', () => document.execCommand('formatBlock', false, 'H2')],
          ['heading', 'Заголовок 3', () => document.execCommand('formatBlock', false, 'H3')],
          ['quote', 'Цитата', () => document.execCommand('formatBlock', false, 'BLOCKQUOTE')],
          ['listUl', 'Маркированный список', () => document.execCommand('insertUnorderedList')],
          ['listOl', 'Нумерованный список', () => document.execCommand('insertOrderedList')],
          ['code', 'Блок кода', () => document.execCommand('formatBlock', false, 'PRE')],
          ['minus', 'Разделитель', () => document.execCommand('insertHorizontalRule')]
        ];

        const toolbar = sheet.querySelector('[data-toolbar]');
        tools.forEach(([icon, label, cmd]) => {
          const btn = el(`<button type="button" aria-label="${label}" title="${label}">${I.get(icon, 18)}</button>`);
        ...
        async function saveNote(silent = false) {
          ...
          // Санитизация при сохранении (защита от внедрения скриптов)
          const body = U.sanitizeHTML(editor.innerHTML.trim());
          if (!title && !body.replace(/<br\s*\/?>/i, '')) return;
        ...
        const item = el(`
          <div class="project-item">
            ...
            <div style="flex:1;min-width:0">
              ...
              <small style="color:var(--text-tertiary);font-size:11px">${done.length}/${total} · ${pct}%</small>
            </div>
      ...
      C.Modal.open({
        ...
        onMount(sheet, close) {
          ...
          S.PROJECT_COLORS.forEach((c, i) => {
            const sw = el(`<button style="width:36px;height:36px;border-radius:50%;background:${c};outline:3px solid transparent;outline-offset:2px" aria-label="${c}"></button>`);
  ...
  Views.stats = function (container) {
    ...
    function renderBody() {
      ...
      body.innerHTML = `
        ...
        <div class="card"><div class="legend">
          ${Object.entries(I.PRIORITY).map(([k, p]) =>
            `<div class="legend-item"><span class="legend-dot" style="background:${p.color}"></span>${p.label}: <strong>${s.byPriority[k]}</strong></div>`
          ).join('')}
        </div></div>
      ...
      for (let i = 181; i >= 0; i--) {
        ...
        const cell = el(`<div class="heatmap-cell lvl${lvl}" title="${iso}: ${count}"></div>`);
        hm.appendChild(cell);
      }
    }
  ...
  Views.settings = function (container) {
    ...
    container.innerHTML = `
      ...
      <div class="settings-group">
        <h2>Данные</h2>
        <button class="settings-item" data-act="export">${I.get('download', 20)} Экспорт данных${I.get('chevronRight', 16, 1.5).replace('<svg', '<svg class="chevron"')}</button>
        <button class="settings-item" data-act="import">${I.get('upload', 20)} Импорт из файла${I.get('chevronRight', 16, 1.5).replace('<svg', '<svg class="chevron"')}</button>
        <button class="settings-item" data-act="trash">${I.get('trash', 20)} Корзина${I.get('chevronRight', 16, 1.5).replace('<svg', '<svg class="chevron"')}</button>
    ...
    themeMeta.forEach(([key, grad, label]) => {
      const sw = el(`<button class="theme-swatch ${st.theme === key ? 'active' : ''}">
        <span class="theme-swatch-preview" style="background:${grad}"></span><span>${label}</span></button>`);
      sw.addEventListener('click', () => {
    ...
    function render() {
      ...
      deleted.forEach((item) => {
        ...
        const row = el(`
          <div class="project-item">
            <div style="flex:1;min-width:0">
              <strong style="font-size:14px;display:flex;align-items:center;gap:6px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
                ${isTask ? I.get('check-square', 14) : I.get('note', 14)} ${esc(item.title || 'Без названия')}
              </strong>
              <small class="trash-expire">удалится через ${daysLeft} ${U.plural(daysLeft, ['день', 'дня', 'дней'])}</small>
            </div>
